...
9. *LogFilesStoringDays* - количество дней хранения файлов журнала в папке *LogFolder* (0 - не удалять)
...
```
где:</br>
1. *ConvertJsonToSeparateTables* - возможность конвертации json объекта в отдельную таблицу </br>

Пример отправки лога в отдельную таблицу:
```json
{"http.log": {
    "BaseURL": "https://ya.ru",
...
В результате будет создана таблица с именем "http_log" и полями: 
(служебные)
  "_event_id Int64", 
...
(Пользовательские)
  "BaseURL String", 
  "Headers String"
Кириллица в ключах преобразуется в транслит.
...
1. *MaximumRetries* - количество попыток переподключения к очередному узлу
2. *MaxRetryTimeout* - таймаут попытки подключения
